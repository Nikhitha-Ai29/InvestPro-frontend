    ...
    <DashboardLayout>
      <div className="portfolio-page">
        <div className="portfolio-header">
          ...
          <button className="btn btn-primary" onClick={() => navigate('/explore')}>+ Invest Funds</button>
        </div>
        ...
        <div className="portfolio-list">
          ...
          {!loading && !error && investments.length === 0 && (
            <div className="empty-state">
              ...
              <button className="btn btn-primary" onClick={() => navigate('/explore')}>Explore Funds</button>
            </div>
          )}
          {!loading && !error && investments.length > 0 && (
                ...
                <div key={inv.id || index} className="investment-card card">
                  ...
                  <div className="investment-details">
                    <div className="detail-row">
                      <span>Type</span>
                      <span className="badge">{(inv.type || inv.investmentType) === 'SIP' ? 'SIP' : 'One Time'}</span>
                    </div>
                    <div className="detail-row">
                      <span>Invested</span>
                      <span>₹{(inv.amount || inv.investment || 0).toFixed(2)}</span>
                    </div>
                    <div className="detail-row">
                      <span>Expected Returns</span>
                      <span className="returns">₹{(inv.returns || inv.expectedReturns || 0).toFixed(2)}</span>
                    </div>
                    <div className="detail-row">
                      <span>Maturity Value</span>
                      <span className="maturity">₹{(inv.maturityValue || inv.maturity || 0).toFixed(2)}</span>
                    </div>
                    <div className="detail-row">
                      <span>Date</span>
                  ...
                  <button className="btn btn-secondary btn-invest-more" onClick={() => navigate('/explore')}>
                    Invest More
                  </button>
                </div>
              ))}
